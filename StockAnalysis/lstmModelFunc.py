...
def train_and_plot_lstm(csv_file, first_date_str, last_date_str, ticker, window_size=3, learning_rate=0.001, epochs=100, sentiment_multiplier=1):
    ...
    # Adjust extrapolated values based on RSI and Bollinger Bands
    extrapolated_df = pd.DataFrame({'Date': extrapolated_dates, 'Close': extrapolated_predictions})
    extrapolated_df['RSI'] = calculate_rsi(extrapolated_df['Close'])
    extrapolated_df['Rolling Mean'], extrapolated_df['Upper Band'], extrapolated_df['Lower Band'] = calculate_bollinger_bands(extrapolated_df['Close'])

    for i in range(len(extrapolated_predictions)):
        if extrapolated_df['RSI'].iloc[i] > 70 or extrapolated_df['Close'].iloc[i] > extrapolated_df['Upper Band'].iloc[i]:
            extrapolated_predictions[i] = extrapolated_df['Rolling Mean'].iloc[i]  # Push to the middle of the Bollinger Bands
    ...
    plt.plot(df.index, df['Upper Band'], label='Upper Bollinger Band', linestyle='dotted')
    plt.plot(df.index, df['Lower Band'], label='Lower Bollinger Band', linestyle='dotted')
